...
GlutApp::GlutApp(int argc, char** argv, int width, int height, const char* title){
    ...
    // hide the Operating Sytem cursor
    //glutSetCursor(GLUT_CURSOR_NONE);        // is supposed to make cursor invisible but does not work on windows with XMing, cannot test on Linux
    ...
    // Set callback to handle mouse movement without clicking
...
void GlutApp::glutPassiveMotionCB(int x, int y) {
    // Convert from window to scene coordinates
    float mx = (float)x;
    float my = (float)y;
    windowToScene(mx, my);

    singleton->moveMouse(mx, my);
}
